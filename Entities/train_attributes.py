...
train_atr, test_atr = pickle.load(open(pickle_directory + '/Attributes/' + '0.pickle', 'rb'))

n_nodes_hl1 = 1500
n_nodes_hl2 = 1500
n_nodes_hl3 = 1500

n_classes = len(train_atr[0])
batch_size = 100
hm_epochs = 10
	...
	with tf.Session() as sess:
		...
		for epoch in range(hm_epochs):
			...
			for file in files:
				...
				train_attributes, test_attributes = pickle.load(open(pickle_directory + '/Attributes/' + file, 'rb'))
				i=0
				while i < len(train_query):
					start = i
					end = i+batch_size
					batch_x = np.array(train_query[start:end])
					batch_y = np.array(train_attributes[start:end])
			...
			print('Epoch', epoch+1, 'completed out of',hm_epochs,'Loss:',epoch_loss,"Accuracy:", accuracy.eval({x: test_q, y: test_atr}))
