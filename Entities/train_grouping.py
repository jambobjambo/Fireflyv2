...
train_grp, test_grp = pickle.load(open(pickle_directory + '/Grouping/' + '0.pickle', 'rb'))

n_nodes_hl1 = 1500
n_nodes_hl2 = 1500
n_nodes_hl3 = 1500

n_classes = len(train_grp[0])
batch_size = 100
hm_epochs = 20
	...
	with tf.Session() as sess:
		...
		for epoch in range(hm_epochs):
			...
			for file in files:
				...
				train_group, test_group = pickle.load(open(pickle_directory + '/Grouping/' + file, 'rb'))
				i=0
				while i < len(train_query):
					start = i
					end = i+batch_size
					batch_x = np.array(train_query[start:end])
					batch_y = np.array(train_group[start:end])
			...
			print('Epoch', epoch+1, 'completed out of',hm_epochs,'Loss:',epoch_loss,"Accuracy:", accuracy.eval({x: test_q, y: test_grp}))
