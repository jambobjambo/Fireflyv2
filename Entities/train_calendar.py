...
train_cl, test_cl = pickle.load(open(pickle_directory + '/Calendar/' + '0.pickle', 'rb'))

n_nodes_hl1 = 1500
n_nodes_hl2 = 1500
n_nodes_hl3 = 1500

n_classes = len(train_cl[0])
batch_size = 100
hm_epochs = 20
	...
	with tf.Session() as sess:
		...
		for epoch in range(hm_epochs):
			...
			for file in files:
				...
				train_calendar, test_calendar = pickle.load(open(pickle_directory + '/Calendar/' + file, 'rb'))
				i=0
				while i < len(train_query):
					start = i
					end = i+batch_size
					batch_x = np.array(train_query[start:end])
					batch_y = np.array(train_calendar[start:end])
			...
			print('Epoch', epoch+1, 'completed out of',hm_epochs,'Loss:',epoch_loss,"Accuracy:", accuracy.eval({x: test_q, y: test_cl}))
			saver.save(sess, "../Checkpoints/Calendar/model.ckpt")
